...
export default class SingleSquareFootView extends React.Component {
  constructor(props) {
    super(props);
  ...
  getExchangeRates = (paymentMode) => {
    ...
    fetch(
      ...
      .then((resJson) => {
        ...
        let productTotalAmount = this.props.arr[this.props.index]["amounts"][
          currencyType
        ][0];
        maxLimit1 = this.props.arr[this.props.index]["maxLimit"][currencyType];
        let emi = productTotalAmount / maxLimit1;
        if (emi % maxLimit1 > 0) {
          emi += 1;
        }
        emi = Number(emi.toString().split(".")[0]);
        if (emi > installmentArr[installmentArr.length - 1]) {
          installmentArr = [];
          installmentArr.push(emi);
        } else {
          for (i = 0; i < installmentArr.length; i++) {
            if (installmentArr[i] > emi) {
              breakIndex = i;
              break;
            }
          }
          installmentArr.splice(0, breakIndex, emi);
        }
        for (i = 0; i < installmentArr.length; i++) {
          if (i == 0) {
            installmentsChechBtn.push(true);
          } else {
            installmentsChechBtn.push(false);
          }
        }
        ...
        this.setState({
          money: (money / installmentArr[0]).toFixed(2),
  ...
  razorPay = () => {
    ...
    if (this.state.customAmountRadio) {
      ...
      amount = (
        Number(this.state.money) * this.state.unitValue * quantity +
        (this.state.checkBtn ? Number(transactionFee) : 0)
      ).toFixed(2);
    }
    var options = {
  ...
  //   let quantity = this.state.quantity ? this.state.quantity : 1;
  //   if (this.state.customAmountRadio) {
  //     amount = Number(this.state.customAmount).toFixed(2);
  //   } else {
  //     amount = Number(this.state.money) * this.state.unitValue * quantity;
  //   }
  //   let transactionFee = this.props.arr[this.props.index].transactionFee[
  ...
  //             ).toFixed(2)
  //           ),
  //       perMonthAmount: Number(Number(amount).toFixed(2)),
  ...
  onSuccess = (mode, transactionId) => {
    ...
    this.props
      .paymentController({
        ...
        campaignId: this.props.campaignId,
        currencyType: this.state.totalPicker,
        count: this.state.quantity ? this.state.quantity : 1,
      })
      .then((data) => {
        if (data) {
          this.setState({
  ...
  multiplyQuantity = () => {
    let pickerMoneyArr2 = [];
    let value = this.state.unit == "sq.m" ? 10 : 1;
    let quantity1 = this.state.quantity ? this.state.quantity : 1;
    let pickerMoneyArr = this.state.moneyArr[this.state.totalPicker];
    if (pickerMoneyArr) {
      for (let i = 0; i < pickerMoneyArr.length; i++) {
        pickerMoneyArr2[i] = (pickerMoneyArr[i] * quantity1 * value)
          .toFixed(2)
          .toString();
      }
      if (this.state.money != pickerMoneyArr2[0]) {
        this.setState({ money: pickerMoneyArr2[0] });
      }
    }

    return pickerMoneyArr2;
  };
  getProfileInfo = () => {
  ...
  render() {
    ...
    //}
    //let pickerMoneyArr = this.multiplyQuantity();
    let paypalMoney = this.state.customAmountRadio
      ? (
          Number(this.state.customAmount) +
          (this.state.checkBtn ? Number(transactionFee) : 0)
        ).toFixed(2)
      ...
      <React.Fragment>
              ...
              <html lang="en">
                ...
                <body onload="document.f1.submit();">
                  ...
                  <form name="f1" id="f1" action=${paypalApiUrl}>
                    ...
                    <input name="count" value="${quantity}" id="count" />
        ...
        >
          ...
          <View //.......................v2
            style={{
              ...
              height:
                screenHeight / 2 +
                320 +
                35 * (this.state.installmentArr.length + 1),
              borderRadius: 10,
              alignSelf: "center",
              elevation: 10,
              marginTop: 10,
          ...
          >
                ...
                {Platform.OS === "android" ? (
                  <Picker
                    ...
                    onValueChange={(itemValue, itemIndex) => {
                      if (itemValue == "sq.m") {
                        this.setState({ unit: itemValue, unitValue: 10 });
                      } else {
                        this.setState({
                          unit: itemValue,
                          unitValue: 1,
                ...
                {this.state.customAmountRadio ? (
                  ...
                  >
                    <TextInput
                      ...
                      onChangeText={(txt) =>
                        this.setState({ customAmount: txt })
                      }
                      keyboardType="numeric"
            ...
            {this.state.installmentArr.length > 0
              ? this.state.installmentArr.map((item, index) => (
                  <View //.......................................Installment
                    key={index.toString()}
                    style={{
                      width: "100%",
            ...
            {this.state.quantity > 40 &&
            this.state.unit == "sq.ft" &&
            !this.state.customAmountRadio ? (
              <View //................errorQuantity
                style={{
                  flex: 1,
              ...
              >
                ...
                >
                  Max 40 quantity allowed
                </Text>
              </View>
            ) : null}
            {this.state.quantity > 4 &&
            this.state.unit == "sq.m" &&
            !this.state.customAmountRadio ? (
              <View
                style={{
                  flex: 1,
                ...
                <TouchableOpacity
                      ...
                      if (
                        ...
                        if (this.state.paymentMode == "paypal") {
                          if (Platform.OS == "ios") {
                            ...
                            }&campaignId=${this.props.campaignId}&imgURL=${
                              this.props.arr[this.props.index].imgs[1]
                            }&currencyType=${
                              this.state.totalPicker
                            }&productId=${
                              this.props.arr[this.props.index]._id
                            }&userId=${this.props.userId}&offsetInHours=${
                          ...
                          let url = `${apiUrl}razorpay/browser?email=${
                            ...
                            Number(amount).toFixed(2)
                          )}&transactionAmount=${amount1}&campaignFlag=${
                            this.props.campaingFlag
                          }&campaignId=${this.props.campaignId}&imgURL=${
                            this.props.arr[this.props.index].imgs[1]
                          }&currencyType=${this.state.totalPicker}&productId=${
                            this.props.arr[this.props.index]._id
                          }&userId=${this.props.userId}&offsetInHours=${
                            new Date().getTimezoneOffset() / 60
                          }&payInFull=${
                  ...
                  style={{
                    ...
                    backgroundColor:
                      ...
                      !(
                        (this.state.quantity >
                        (this.state.unit == "sq.ft" ? 40 : 4)
                          ? true
                          : false) && !this.state.customAmountRadio
                      ) &&
